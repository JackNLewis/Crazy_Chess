...
public class PowerupFreeCard
{
	...
	public AbstractPiece[][] freecard(AbstractPiece[][] gamestate, Position king, Position target, Boolean isDebug)
	{
		...
		{
			if (isDebug)
				System.out.println("Targets for swapping are coordinates that are not on the board. Returning NULL");
			return null;
		}
		...
		{
			if (isDebug)
				System.out.println("Must select your king. Returning NULL");
			return null;
		}
		if (!((blank.getXpos() >= king.getXpos() - 2) && (blank.getXpos() <= king.getXpos() + 2) && (blank.getYpos() <= king.getYpos() + 2)
				&& (blank.getYpos() >= king.getYpos() - 2)))
		{
			if (isDebug)
				System.out.println("Not 5X5. Returning NULL");
			return null;
		}
		if (!blank.getColor().equalsIgnoreCase("blank"))
		{
			if (isDebug)
				System.out.println("Cant move to other pieces' position which is not blank. Returning NULL");
			return null;
		}
		if (!ecat.isInCheck(newking.getColor(), isDebug, gamestateCopy, main.getTurnNo()))
		{
			if (isDebug)
				System.out.println("Cant move the king which is not in check. Returning NULL");
			return null;
		}
		if (!(ecat.BlankcapturableBy(blank, newking.getColor(), isDebug, gamestateCopy, main.getTurnNo()).isEmpty()))
		{
			if (isDebug)
				System.out.println("Cant move the king to a place which is in check. Returning NULL");
			return null;
		}
