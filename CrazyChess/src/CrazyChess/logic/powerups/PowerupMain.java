...
/**
 * Class for the main logic of the power up mechanic.
 *
 */
...
public class PowerupMain
{
	 ...
	 * @param target2
	 *            position of the second piece (can be NULL)
	 * @param turnNo
	 *            number representing which turn is it
	 * @param currentTurn
	 *            string representing which player's turn is it (black or white)
	 * @param isDebug
	 *            is debug mode active
	 * @return a gamestate where the effect of the powerup has been resolved
	 */
	public AbstractPiece[][] powerupAssigner(String powerup, AbstractPiece[][] gamestate, Position target1, Position target2, int turnNo,
			String currentTurn, boolean isDebug)
	{
	...
	/**
	 * Returns a string representation of a random powerup. Should be used to get a random powerup when a powerup piece gets captured on the
	 * board, then added to whitePowerUps or blackPowerUps ArrayList.
	 * 
	 * @param isDebug
	 *            - if debug is on
	 *
	...
	public String randomPowerup(boolean isDebug)
	{
		...
		switch (random)
		{ //Add a case for each new powerup
		case 1:
			return "Teleport";
		case 2:
			return "FreeCard";
		...
		if (isDebug)
			System.out.println("Something went wrong when returning a random powerup. Returning NULL");
		return null;
	}

	/**
	 * A method to get all of the possible moves you can do with a power up.
	 * 
	 * @param powerup
	 *            the powerup to be used
	 * @param gamestate
	 *            current gamestate
	 * @param target1
	 *            position of the piece the powerup is targeting
	 * @param isDebug
	 *            is debug mode on
	...
	public ArrayList<Position> validPowerupMoves(String powerup, AbstractPiece[][] gamestate, Position target1, boolean isDebug)
	{
		...
		if (powerup.equalsIgnoreCase("teleport"))
		{
			...
			if (target.getColor().equalsIgnoreCase("white"))
			{
				...
				{
					if (!p.getPosition().equals(target.getPosition()))
					{//add all of the positions of the pieces of the same color as target, except the position of target itself
		...
		else if (powerup.equalsIgnoreCase("freecard"))
		{
			...
			for (AbstractPiece p : Bp)
			{
				if ((p.getXpos() >= target.getXpos() - 2) && (p.getXpos() <= target.getXpos() + 2) && (p.getYpos() <= target.getYpos() + 2)
						&& (p.getYpos() >= target.getYpos() - 2))
				{
					moves.add(p.getPosition());
				}
			}
	 ...
	 * 
	 * @param newGamestate
	 *            gamestate to be edited
	 * @param newPiece
	 *            piece that had the bomb on it
	 * @param copiedPiece
	 *            copy of a piece that had a bomb on it
	 * @param utils
	 ...
	 */
	public AbstractPiece[][] doBomb(AbstractPiece[][] newGamestate, AbstractPiece newPiece, AbstractPiece copiedPiece, Utilities utils,
			boolean isDebug)
	{
		if (utils.isOnBoard(newPiece.getXpos() - 1, newPiece.getYpos() - 1)
			...
			newGamestate = utils.placePiece(new BlankPiece("Blank", newPiece.getXpos() - 1, newPiece.getYpos() - 1, "Normal"), isDebug,
					newGamestate);
	 ...
	 */
	public AbstractPiece[][] usePowerupGivenGamestate(String pwrUpStr, AbstractPiece[][] gamestate, String currentTurn, Position target1,
			Position target2, boolean isDebug)
	{

		AbstractPiece[][] copiedGamestate = utils.safeCopyGamestate(gamestate);
		AbstractPiece[][] gamestateAfterPowerup = powerupAssigner(pwrUpStr.toLowerCase(), copiedGamestate, target1, target2, 0, currentTurn,
				isDebug);
