...
public class PowerupMiniPromote
{
	 ...
	 * @param target1
	 *            first piece to be Promoted
	 * @param target2
	 *            second piece that first piece will to be
	 * @param isDebug
	...
	public AbstractPiece[][] promote(AbstractPiece[][] gamestate, Position target1, Position target2, Boolean isDebug)
	{
		...
		{
			if (isDebug)
				System.out.println("Target for Promotation is coordinate that is not on the board. Returning NULL");
			return null;
		}
		...
		{
			if (isDebug)
				System.out.println("Can't promote piece which is a DummyPiece. Returning NULL");
			return null;
		}
		if (!(piece1 instanceof Pawn))
		{
			if (isDebug)
				System.out.println("Can't promote piece which is not Pawn. Returning NULL");
			return null;
		}
		if (!(piece2 instanceof Bishop) && !(piece2 instanceof Knight))
		{
			if (isDebug)
				System.out.println("The pawn only can turn into Bishop or Knight. Returning NULL");
			return null;
		}
		if (piece1.getColor().equalsIgnoreCase("blank"))
		{
			if (isDebug)
				System.out.println("Can't use blank pieces in teleportation. Returning NULL");
			return null;
		}
