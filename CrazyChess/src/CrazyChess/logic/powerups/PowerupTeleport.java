...
public class PowerupTeleport
{
	...
	public AbstractPiece[][] teleport(AbstractPiece[][] gamestate, Position target1, Position target2, Boolean isDebug)
	{
		...
		{
			if (isDebug)
				System.out.println("Targets for teleportation are coordinates that are not on the board. Returning NULL");
			return null;
		}
		...
		{
			if (isDebug)
				System.out.println("Can't swap kings. Returning NULL");
			return null;
		}
		if (!piece1.getColor().equalsIgnoreCase(piece2.getColor()))
		{
			if (isDebug)
				System.out.println("You can only swap pieces of your own color. Returning NULL");
			return null;
		}
