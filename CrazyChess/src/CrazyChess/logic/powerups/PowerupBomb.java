...
public class PowerupBomb
{
	...
	public AbstractPiece[][] bomb(AbstractPiece[][] gamestate, Position target1, Position target2, Boolean isDebug)
	{
		...
		{
			if (isDebug)
				System.out.println("Target for Bomb is coordinate that is not on the board. Returning NULL");
			return null;
		}
		...
		for (AbstractPiece x : Bx)
		{
			if ((x.getPoweruptype().equalsIgnoreCase("bomb")))
			{
				...
				{
					if (isDebug)
						System.out.println("Can have two or more bomb!");
					return null;
				}
			}
		}
		...
		{
			if (isDebug)
				System.out.println("Can't add Bomb on a piece which is not pawn. Returning NULL");
			return null;
		}
		...
		{
			if (isDebug)
				System.out.println("Can't add Bomb on the blank. Returning NULL");
			return null;
		}
