...
/**
 * This class performs some basic checks to see if a move on the board is at least somewhat valid. i.e. doesn't go through other pieces,
...
public class BasicValidityChecker
{
	 ...
	 * 
	 * @param p
	 *            input piece that is being checked
	 * @param xRel
	...
	/**
	 * Return furthest possible tile if moving in a column or row. Further used when checking possible moves of rooks and queens
	 * 
	 * @param x
	 *            x coordinate of the checked position
	 * @param y
	 *            y coordinate of the checked position
	 * @param dir
	 *            string representing direction: up, down, left, or right
	 * @param p
	 *            initial piece being checked, doesn't change, just used
	 * @param newPos
	 *            the new Position
	 * @param isDebug
	 ...
	 */
	public Position columnAndRowCheck(int x, int y, String dir, AbstractPiece p, Position newPos, boolean isDebug,
			AbstractPiece[][] gamestate)
	{
		if (dir == "up")
		{
			...
			{//if none of the conditions are met, the function recurses with the next tile in the 
	...
	/**
	 * Returns furthest possible diagonal placement. Further used for bishops ands queens
	 * 
	 * @param x
	 *            x coordinate of the checked position
	 * @param y
	 *            y coordinate of the checked position
	 * @param dir
	 *            diagonal directions as strings
	 * @param p
	 *            initial piece being checked, doesn't change, just used
	 * @param newPos
	 *            the new Position
	...
	public boolean validityCheckPawn(Pawn p, int xRel, int yRel, boolean isDebug, AbstractPiece[][] gamestate, int turnNo)
	{
		if (p.getColor().equalsIgnoreCase("black"))
		{
			//basic movement
			if (pawnswitch)
			{ //if rulechange2 is active
				if (xRel == 0 && (yRel == -1 || yRel == 1)
						&& !(utils.getTargetPiece(p, xRel, yRel, isDebug, gamestate).getColor().equalsIgnoreCase("white")
			...
			//first turn jump
			if (xRel == 0 && yRel == -2 && p.getYpos() == 6
					&& utils.getPiece(p.getXpos() + xRel, p.getYpos() + yRel, isDebug, gamestate).getColor().equalsIgnoreCase("blank")
		...
		else if (p.getColor().equalsIgnoreCase("white"))
		{
			...
			{ //if rulechange2 is active
				if (xRel == 0 && (yRel == -1 || yRel == 1)
						&& !(utils.getTargetPiece(p, xRel, yRel, isDebug, gamestate).getColor().equalsIgnoreCase("black")
								|| utils.getTargetPiece(p, xRel, yRel, isDebug, gamestate).getColor().equalsIgnoreCase("powerup")))
					return true;
			}
			else
	...
	/**
	 * Checks if Rook's move is legal. Checks all legal tiles that the rook can move to and then checks if the candidate position is among
	 * them.
	 * 
	 * @param p
	 *            the piece to be checked
	...
	/**
	 * Checks if Bishops's move is legal. Checks all legal tiles that the bishop can move to and then checks if the candidate position is
	 * among them.
	 * 
	 * @param p
	 *            the piece to be checked
	...
	/**
	 * Checks if King's move is legal. Checks all legal tiles that the king can move to and then checks if the candidate position is among
	 * them.
	 * 
	 * @param p
	 *            the piece to be checked
	...
	public boolean validityCheckKing(King p, int xRel, int yRel, boolean isDebug, AbstractPiece[][] gamestate)
	{
		...
		AbstractPiece newPos = utils.getTargetPiece(p, xRel, yRel, isDebug, gamestate);
		if (p.getXpos() + 1 == newPos.getXpos() && p.getYpos() + 0 == newPos.getYpos())
		{//(1,0)
			return true;
		}
		else if (p.getXpos() + 1 == newPos.getXpos() && p.getYpos() - 1 == newPos.getYpos())
		{//(1,-1)
			return true;
		}
		else if (p.getXpos() + 0 == newPos.getXpos() && p.getYpos() - 1 == newPos.getYpos())
		{//(0,-1)
			return true;
		}
		else if (p.getXpos() - 1 == newPos.getXpos() && p.getYpos() - 1 == newPos.getYpos())
		...
		else if (p.getXpos() - 1 == newPos.getXpos() && p.getYpos() + 0 == newPos.getYpos())
		{//(-1,0)
			return true;
		}
		else if (p.getXpos() - 1 == newPos.getXpos() && p.getYpos() + 1 == newPos.getYpos())
	...
	/**
	 * Checks if Queen's move is legal. Checks all legal tiles that the queen can move to and then checks if the candidate position is among
	 * them.
	 * 
	 * @param p
	 *            the piece to be checked
	...
	/**
	 * Checks if Knight's move is legal. Checks all legal tiles that the knight can move to and then checks if the candidate position is
	 * among them.
	 * 
	 * @param p
	 *            the piece to be checked
	...
	{
		AbstractPiece newPos = utils.getTargetPiece(p, xRel, yRel, isDebug, gamestate);
		if (p.getXpos() + 2 == newPos.getXpos() && p.getYpos() + 1 == newPos.getYpos())
		{ //    (2,1)
			return true;
		}
		else if (p.getXpos() + 2 == newPos.getXpos() && p.getYpos() - 1 == newPos.getYpos())
		{//(2,-1)
			return true;
		}
		else if (p.getXpos() - 2 == newPos.getXpos() && p.getYpos() + 1 == newPos.getYpos())
		...
		else if (p.getXpos() + 1 == newPos.getXpos() && p.getYpos() + 2 == newPos.getYpos())
		{//(1,2)
			return true;
		}
		else if (p.getXpos() + 1 == newPos.getXpos() && p.getYpos() - 2 == newPos.getYpos())
	...
	/**
	 * Checks if King's move is legal if rulechange3 is active. Checks all legal tiles that the queen can move to and then checks if the
	 * candidate position is among them.
	 * 
	 * @param p
	 *            the piece to be checked
