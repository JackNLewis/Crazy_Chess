...
/**
 * This class is used to perform some extra checks and to provide some extra tools that might be useful further in the project. This class
 * might be very useful for people making the AI because of methods like canCapture, capturableBy and possibleGamestatesAfterNextMove. This
 * class also holds methods for checking for checks and checkmates.
 * 
 * 
 *
 */
...
public class ExtraChecksAndTools
{
	...
	/**
	 * Function that returns and ArrayList of pieces from a given game state
	 * 
	 * @param gamestate
	 *            any game state
	...
	/**
	 * Function that returns an ArrayList of hazard pieces from a given game state
	 * 
	 * @param gamestate
	 *            any game state
	...
	/**
	 * Function that returns and ArrayList of blank pieces from a given game state
	 * 
	 * @param gamestate
	 *            any game state
	 * @return An ArrayList of pieces in the game state
	...
	/**
	 * Function that returns and ArrayList of white pieces from a given game state
	 * 
	 * @param gamestate
	 *            any game state
	...
	/**
	 * Function that returns and ArrayList of black pieces from a given game state
	 * 
	 * @param gamestate
	 *            any game state
	...
	/**
	 * A checker for whether or not the selected piece can move. Does not account for uncovered checks.
	 * 
	 * @param p
	 *            the selected piece
	 * @param isDebug
	 ...
	 * @param gamestate
	 *            game state to be curently examined
	 * @param moveNo
	 *            current move number
	...
	public boolean canMove(AbstractPiece p, boolean isDebug, AbstractPiece[][] gamestate, int moveNo)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			for (int j = 0; j < 8; j++)
			{
				...
				{
					if (bvc.moveCheckAssigner(p, targetTile.getXpos() - p.getXpos(), targetTile.getYpos() - p.getYpos(), isDebug, gamestate,
							moveNo))
					{
						if (!targetTile.getColor().equalsIgnoreCase(p.getColor()))
						{ //checks if the candidate tile doesn't have a piece of the same color on it
							return true;
						}
					}
	...
	/**
	 * Checks if the inputed attacker piece can capture the inputed defender piece
	 * 
	 * @param attacker
	 *            attacking piece
	 * @param defender
	 ...
	 *            is debug mode active
	 * @param gamestate
	 *            game state to be currently examined
	 * @param moveNo
	 *            current move number
	...
	public boolean canCapture(AbstractPiece attacker, AbstractPiece defender, boolean isDebug, AbstractPiece[][] gamestate, int moveNo)
	{
		if (canMove(attacker, isDebug, gamestate, moveNo) == false || !utils.isOnBoard(defender.getPosition())) //primary basic checks 
			return false;
		// Do move check
		if (!bvc.moveCheckAssigner(attacker, defender.getXpos() - attacker.getXpos(), defender.getYpos() - attacker.getYpos(), isDebug,
				gamestate, moveNo))
		{
			return false;
		} //the 2 if statements bellow check if attacker and defender are different colors
		if ((defender.getColor().equalsIgnoreCase("white") || defender.getColor().equalsIgnoreCase("powerup"))
				&& attacker.getColor().equalsIgnoreCase("black"))
		{
			return true;
		}
		else if ((defender.getColor().equalsIgnoreCase("black") || defender.getColor().equalsIgnoreCase("powerup"))
				&& attacker.getColor().equalsIgnoreCase("white"))
		{
			return true;
		}
	 ...
	 *            is debug mode active
	 * @param gamestate
	 *            game state currently to be examined
	 * @param moveNo
	 *            current move number
	...
	public ArrayList<AbstractPiece> BlankcapturableBy(AbstractPiece target, String color, boolean isDebug, AbstractPiece[][] gamestate,
			int moveNo)
	{
		if (!utils.isOnBoard(target.getXpos(), target.getYpos()) || target == null) //checks if target has legal coordinates
			return null;
	 ...
	 * 
	 * @param color
	 *            string of the color of king needed
	 * @param gamestate
	 *            game state currently to be examined
	...
	/**
	 * 
	 * Checks if the king of the specified color is in check in the game state provided
	 * 
	 * @param color
	 *            color of the king to be examined
	 * @param isDebug
	 *            is debug mode active
	 * @param gamestate
	 *            game state to be currently examined
	...
	/**
	 * Returns a list of moves that a piece p can make Does not account for checks and/or uncovered checks
	 * 
	 * @param p
		...
		for (int i = 0; i < 8; i++)
		{
			for (int j = 0; j < 8; j++)
			{
				...
				if (!(p.getXpos() == targetTile.getXpos() && p.getYpos() == targetTile.getYpos()))
				{
					if (bvc.moveCheckAssigner(p, targetTile.getXpos() - p.getXpos(), targetTile.getYpos() - p.getYpos(), isDebug, gamestate,
							moveNo)
							|| ((p instanceof King) && (cstl.castleCheck((King) p, targetTile.getXpos() - p.getXpos(),
									targetTile.getYpos() - p.getYpos(), isDebug, gamestate, moveNo))))
					{
						if (p instanceof King)
						...
						{ //checks if the candidate tile doesn't have a piece of the same color on it
							...
							if (!isInCheck(p.getColor(), isDebug, newGamestate, moveNo))
							{//check if the new possition doesn't put the player in check
	...
	/**
	 * This function returns all possible game states after one turn in the game state inputed with powerups
	 * 
	 * @param whoseTurn
	 *            string of a color of the player whose turn is currently happening
	 * @param isDebug
	 *            is debug mode active
	 * @param gamestate
	 *            starting game state
	 ...
	 *            current move number
	 * @param powerUps
	 *            the arraylist of powerups
	 * @return ArrayList of possible game states after the turn is completed with powerup
	 */

	public HashMap<AbstractPiece[][], Integer> possibleGamestatesAfterNextMove(String whoseTurn, boolean isDebug,
			AbstractPiece[][] gamestate, int moveNo, ArrayList<String> powerUps)
	{
		...
		for (AbstractPiece p : piecesToCheck)
		{
			...
			for (Position vp : validPositions)
			{
				...
				AbstractPiece[][] newGamestate = utils.safeCopyGamestate(gamestate);
				if (!(vp.getXpos() == getKing(oppColor, newGamestate).getXpos()
						&& vp.getYpos() == getKing(oppColor, newGamestate).getYpos()))
				{
			...
			for (int i = 0; i < powerUps.size(); i++)
			{
				...
				if (!validPowerupMoves.isEmpty())
				{
					...
					{
						AbstractPiece[][] modifiedGames = pwrUp.usePowerupGivenGamestate(pwrUpStr, copiedGamestate, p.getColor(),
								p.getPosition(), finalPos, isDebug);
		...
		{
			if (isDebug)
			{ //if the code is working, it should never reach this line of the method
	...
	/**
	 * This method checks if the player of specified color is in checkmate in the current game state
	 * 
	 * @param color
	 *            color of the player being examined for checkmate
	 * @param isDebug
	 *            is debug mode activated
	 * @param gamestate
	 *            game state to be examined
	 ...
	 * 
	 * @param currentTurn
	 *            color of the player the moves recently
	 * @param isDebug
	 *            is debug mode activated
	 * @param gamestate
	 *            game state to be examined
	...
	public boolean isInDraw(String currentTurn, boolean isDebug, AbstractPiece[][] gamestate, int moveNo, ArrayList<String> powerUps)
	{
		...
		HashMap<AbstractPiece[][], Integer> nextMoves = possibleGamestatesAfterNextMove(utils.oppositeColor(currentTurn), isDebug,
				gamestate, moveNo, powerUps);
