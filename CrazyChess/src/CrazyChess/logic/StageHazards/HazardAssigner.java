 ...
 */
	...
	private AbstractPiece[][] burnHazard(AbstractPiece[][] gameState) {
		...
		for (int i = 0; i < getThreshhold(blankPieces.size()); i++) {
			...
			HazardPiece burnTile = new HazardPiece(blankPieces.get(index).getPosition(), Hazard.BURN,
					blankPieces.get(index));
 ...
 * @param size     the number of tiles a stage hazard could affect
