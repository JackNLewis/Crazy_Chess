...
/**
 * This is the main class for the chess game logic. Ties everything to do with logic together.
 * 
 *
 */
...
public class MainLogic
{
	...
	/**
	 * Constructor for the MainLogic class. Initiates the gamestate as an empty board. Initiates currentTurn with "white" Initiates turnNo
	 * with 1
	 */
	...
	/**
	 * Method used to see if a player of specified color in currently under check
	 * 
	 * @param color
	 *            color of the player to check
	...
	/**
	 * Method used to see if a player of specified color in currently mated
	 * 
	 * @param color
	 *            color of the player to check
	 ...
	 * 
	 * @param currentTurn
	 *            color of the player who's turn to switct to
	 */
	...
	/**
	 * Changes the turn to the opposite of what it currently is. If for some reason the turn is neither Black nor White by default the
	 * method sets it to white
	 */
			...
			for (AbstractPiece x : Bx)
			{
				if ((x.getPoweruptype().equalsIgnoreCase("bomb")))
				{
					if (x.getColor().equalsIgnoreCase("white") && (turnNo - WBlt >= 5))
					{
						...
						if (utils.isOnBoard(x.getXpos() - 1, x.getYpos() - 1)
								&& !((utils.getPiece(x.getXpos() - 1, x.getYpos() - 1, isDebug, gamestate)) instanceof King))
							gamestate = utils.placePiece(new BlankPiece("Blank", x.getXpos() - 1, x.getYpos() - 1, "Normal"), isDebug,
									gamestate);
						if (utils.isOnBoard(x.getXpos() - 1, x.getYpos())
								&& !((utils.getPiece(x.getXpos() - 1, x.getYpos(), isDebug, gamestate)) instanceof King))
							gamestate = utils.placePiece(new BlankPiece("Blank", x.getXpos() - 1, x.getYpos(), "Normal"), isDebug,
									gamestate);
						if (utils.isOnBoard(x.getXpos(), x.getYpos() - 1)
								&& !((utils.getPiece(x.getXpos(), x.getYpos() - 1, isDebug, gamestate)) instanceof King))
							gamestate = utils.placePiece(new BlankPiece("Blank", x.getXpos(), x.getYpos() - 1, "Normal"), isDebug,
									gamestate);
						if (utils.isOnBoard(x.getXpos(), x.getYpos())
								&& !((utils.getPiece(x.getXpos(), x.getYpos(), isDebug, gamestate)) instanceof King))
							gamestate = utils.placePiece(new BlankPiece("Blank", x.getXpos(), x.getYpos(), "Normal"), isDebug, gamestate);
						if (utils.isOnBoard(x.getXpos() + 1, x.getYpos() - 1)
								&& !((utils.getPiece(x.getXpos() + 1, x.getYpos() - 1, isDebug, gamestate)) instanceof King))
							gamestate = utils.placePiece(new BlankPiece("Blank", x.getXpos() + 1, x.getYpos() - 1, "Normal"), isDebug,
									gamestate);
	...
	/**
	 * Switches the turn to the opposite of what it currently is, but does not increase the number of turns. Used for the "ask for draw"
	 * button. If for some reason the turn is neither Black nor White by default the method sets it to white
	 */
	...
	public void resetBoard()
	{
		...
		{
			for (int j = 0; j < 8; j++)
			{// first replaces all pieces with blank
	...
	/**
	 * This method defines a move. Takes all of the rules and restrictions into consideration. Moves a piece x squares right/left and y
	 * squares up/down. Further used in function moveTo
	 *
	 * @param p
	 *            the piece selected to be moved
	 * @param xRel
	 *            how far left or right to move
	 * @param yRel
	 *            how far up or down to move
		...
		if (isValid)
		{
			...
			if (p instanceof Pawn && ((Pawn) p).getEnPassant() == true)
			{
				if (p.getColor().equalsIgnoreCase("white"))
					newGamestate = utils.placePiece(new BlankPiece("Blank", copiedPiece.getXpos(), (copiedPiece.getYpos() - 1), "Normal"),
							isDebug, newGamestate);
				else
					newGamestate = utils.placePiece(new BlankPiece("Blank", copiedPiece.getXpos(), (copiedPiece.getYpos() + 1), "Normal"),
							isDebug, newGamestate);
			}
			...
			{
				System.out.println("Bomb!!!");
				if (utils.isOnBoard(newPiece.getXpos() - 1, newPiece.getYpos() - 1)
						&& !((utils.getPiece(newPiece.getXpos() - 1, newPiece.getYpos() - 1, isDebug, newGamestate)) instanceof King))
					newGamestate = utils.placePiece(new BlankPiece("Blank", newPiece.getXpos() - 1, newPiece.getYpos() - 1, "Normal"),
							isDebug, newGamestate);
				if (utils.isOnBoard(newPiece.getXpos() - 1, newPiece.getYpos())
						&& !((utils.getPiece(newPiece.getXpos() - 1, newPiece.getYpos(), isDebug, newGamestate)) instanceof King))
					newGamestate = utils.placePiece(new BlankPiece("Blank", newPiece.getXpos() - 1, newPiece.getYpos(), "Normal"), isDebug,
							newGamestate);
				if (utils.isOnBoard(newPiece.getXpos(), newPiece.getYpos() - 1)
						&& !((utils.getPiece(newPiece.getXpos(), newPiece.getYpos() - 1, isDebug, newGamestate)) instanceof King))
					newGamestate = utils.placePiece(new BlankPiece("Blank", newPiece.getXpos(), newPiece.getYpos() - 1, "Normal"), isDebug,
							newGamestate);
				if (utils.isOnBoard(newPiece.getXpos() + 1, newPiece.getYpos() - 1)
						&& !((utils.getPiece(newPiece.getXpos() + 1, newPiece.getYpos() - 1, isDebug, newGamestate)) instanceof King))
					newGamestate = utils.placePiece(new BlankPiece("Blank", newPiece.getXpos() + 1, newPiece.getYpos() - 1, "Normal"),
				...
				if (!(copiedPiece instanceof King))
				{
					if (utils.isOnBoard(newPiece.getXpos(), newPiece.getYpos())
							&& !((utils.getPiece(newPiece.getXpos(), newPiece.getYpos(), isDebug, newGamestate)) instanceof King))
						newGamestate = utils.placePiece(new BlankPiece("Blank", newPiece.getXpos(), newPiece.getYpos(), "Normal"), isDebug,
								newGamestate);
				}
				else if ((copiedPiece instanceof King))
				{
					if (utils.isOnBoard(newPiece.getXpos(), newPiece.getYpos())
							&& !((utils.getPiece(newPiece.getXpos(), newPiece.getYpos(), isDebug, newGamestate)) instanceof King))
						newGamestate = utils.placePiece(copiedPiece, isDebug, newGamestate);
				}
			...
			if (currentTurn.equalsIgnoreCase("white"))
			{
				if (ecat.isInCheckmate("black", isDebug, newGamestate, turnNo + 1, getPowerUps(currentTurn)))
				{
					...
					System.out.println(ecat.validMoves(ecat.getKing("black", newGamestate), isDebug, newGamestate, turnNo));
			}
			if (currentTurn.equalsIgnoreCase("black"))
			{
				if (ecat.isInCheckmate("white", isDebug, newGamestate, turnNo + 1, getPowerUps(currentTurn)))
				{
					...
					System.out.println(ecat.validMoves(ecat.getKing("white", newGamestate), isDebug, newGamestate, turnNo).size());
			}
	 ...
	 *            the piece selected to be moved
	 * @param x
	 *            x position to attempt to move to
	 * @param y
	 *            y position to attempt to move to
	 * @return true if move was successful, false if not
	 */
	...
	/**
	 * Basically just a neat little wrapper for getPiece method in CrazyChess.logic.Utilities
	 * 
	 * @param pos
	 *            target position
	...
	/**
	 * Just a wrapper for the printGameState method in the Utilities class
	 * 
	 */
	...
	/**
	 * Method for using powerups. Returns true if the use of the powerup process was successful, false if not. If using a powerup was
	 * successful, it will also alter the current gamestate.
	 * 
	 * @param powerupIndex
	 *            index (in whitePowerUps or blackPowerUps) of the powerup to be used
	 * @param target1
	 *            position of the first piece to be used in the powerup
	 * @param target2
	 *            position of the second piece to be used in the powerup (can be NULL)
	...
	public boolean usePowerup(int powerupIndex, Position target1, Position target2)
	{
		...
		if (currentTurn.equalsIgnoreCase("white"))
		{
			...
			listToUse = blackPowerUps;

		AbstractPiece[][] copiedGamestate = utils.safeCopyGamestate(gamestate);

		AbstractPiece[][] gamestateAfterPowerup = pwrUp.powerupAssigner(listToUse.get(powerupIndex).toLowerCase(), copiedGamestate, target1,
				target2, turnNo, currentTurn, isDebug);

		if (gamestateAfterPowerup != null)
		{
			if (isDebug)
				System.out.println(currentTurn + " just used a powerup: " + listToUse.get(powerupIndex));
	 ...
	 * 
	 * @param player
	 *            desired player's color
	 * @param check
	 *            check status (true if the player is checked, false if not)
	 */
	 ...
	 *            desired player's color
	 * @param mate
	 *            check status (true if the player is mated, false if not)
	 */
	...
	/**
	 * Gets the value for WB. The value is used to check if a bomb explosion sound is supposed to be played for white.
	 * 
	...
	/**
	 * Gets the value for BB. The value is used to check if a bomb explosion sound is supposed to be played for black.
	 * 
	 ...
	 * 
	 * @param rulechange1
	 *            value for the rulechange (true if active, false if not)
	 */
