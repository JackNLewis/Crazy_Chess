...
/**
 * Class with some utilities for getting pieces from and placing pieces to game states
 * 
 * 
 *
 */
...
public class Utilities
{
	 ...
	 * 
	 * @param pos
	 *            the position object to retrieve located piece
	 * @param isDebug
	 *            isDebug mode activated
	 * @param gamestate
	 *            current game state
	...
	public AbstractPiece getPiece(Position pos, boolean isDebug, AbstractPiece[][] gamestate)
	{
		...
		return pos.getXpos() > 7 || pos.getXpos() < 0 || pos.getYpos() > 7 || pos.getYpos() < 0 ? null
				: gamestate[pos.getXpos()][pos.getYpos()];
	}

	/**
	 * Very brute function to place a piece in the game state and return the new game state. Does not take into consideration any
	 * restrictions or rules on movement, will literally place any piece anywhere on the board. Should be mostly used on empty boards, or if
	 * 100% sure that the tile in which the piece is about to placed is unoccupied. !!!BE VERY CAREFUL WHEN USING THIS!!!
	 * 
	 * @param p
	 *            piece to be place
	 * @param isDebug
	...
	public AbstractPiece[][] placePiece(AbstractPiece p, boolean isDebug, AbstractPiece[][] gamestate)
	{
		...
		{//checks if the piece coordinates are not off the board
			if (isDebug)
	...
	/**
	 * Returns a piece acquired from adding some some offset values x and y to the coordinates of the starting piece. Further used to check
	 * what piece (if any) is in the square another piece is about to go to.
	 * 
	 * @param p
	 *            the starting piece to check from
	 * @param x
	 ...
	 * 
	 * @param pos
	 *            position of the starting piece
	 * @param x
	 *            the up and down movement
	 * @param y
	 *            the left and right movement
	...
	/**
	 * Very brute function to place a piece in the specified coordinates x and y of game state and return the new game state. Does not take
	 * into consideration any restrictions or rules on movement, will literally place any piece anywhere on the board. Should be mostly used
	 * on empty boards, or if 100% sure that the tile in which the piece is about to placed is unoccupied. !!!BE VERY CAREFUL WHEN USING
	 * THIS!!!
	 * 
	 * @param p
	 *            piece to be place
	...
	{
		if (!isOnBoard(p.getPosition()) && !isOnBoard(new Position(x, y)))
		{//checks if the piece coordinates and target tile are not off the board
			if (isDebug)
				System.out.println("Invalid piece place!");
			return null;
		}
	...
	/**
	 * Very brute function to place a piece in the specified position pos of the game state and return the new game state. Does not take
	 * into consideration any restrictions or rules on movement, will literally place any piece anywhere on the board. Should be mostly used
	 * on empty boards, or if 100% sure that the tile in which the piece is about to placed is unoccupied. !!!BE VERY CAREFUL WHEN USING
	 * THIS!!!
	 * 
	...
	{
		if (!isOnBoard(p.getPosition()) && !isOnBoard(pos))
		{//checks if the piece coordinates and target tile are not off the board
			if (isDebug)
				System.out.println("Invalid piece place!");
			return null;
	...
	/**
	 * Very brute function to move a piece in the game state and return the new game state. Does not take into consideration any
	 * restrictions or rules on movement, will literally move any piece anywhere on the board.
	 * 
	 ...
	 *            piece to be moved
	 * @param gamestate
	 *            game state to be edited
	 * @param xNew
	 *            x coordinate of piece's new position
	 * @param yNew
	 *            y coordinate of piece's new position
	...
	/**
	 * Very brute function to move a piece in the game state and return the new game state. Does not take into consideration any
	 * restrictions or rules on movement, will literally move any piece anywhere on the board. !!!BE VERY CAREFUL WHEN USING THIS!!!
	 * 
	 * @param p
	 *            piece to be moved
	 * @param gamestate
